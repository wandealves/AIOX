...
export const TOOL_TEMPLATES: ToolTemplate[] = [
  ...
  },
  {
    id: "email-smtp",
    name: "Email (SMTP)",
    description: "Send emails via SMTP. The AI composes and sends emails based on user instructions",
    category: "communication",
    iconName: "Mail",
    authType: "api_key",
    defaults: {
      endpoint_url: "builtin://smtp_send_email",
      http_method: "POST",
      parameters: { type: "object", properties: { to: { type: "string", description: "Recipient email address(es), separated by commas" }, subject: { type: "string", description: "Email subject line" }, body: { type: "string", description: "Email body content (plain text or HTML depending on content_type)" }, cc: { type: "string", description: "CC recipient(s), separated by commas" }, bcc: { type: "string", description: "BCC recipient(s), separated by commas" }, reply_to: { type: "string", description: "Reply-To email address" }, content_type: { type: "string", enum: ["text", "html"], description: "Content type: 'text' (default) or 'html'", default: "text" } }, required: ["to", "subject", "body"] },
      timeout_sec: 30,
    },
    configFields: [
      { key: "smtp_host", label: "SMTP Host", type: "text", required: true, placeholder: "smtp.gmail.com", helpText: "SMTP server hostname" },
      { key: "smtp_port", label: "SMTP Port", type: "number", required: false, defaultValue: "587", helpText: "587 for STARTTLS, 465 for SSL" },
      { key: "smtp_user", label: "SMTP User", type: "text", required: true, placeholder: "user@example.com", helpText: "SMTP authentication username" },
      { key: "smtp_password", label: "SMTP Password", type: "password", required: true, placeholder: "App password or SMTP password", helpText: "SMTP authentication password (use app passwords for Gmail)" },
      { key: "from_address", label: "From Address", type: "text", required: false, placeholder: "noreply@example.com", helpText: "Sender email address (defaults to SMTP user if empty)" },
    ],
    buildRequest: (v) => ({
      ...
      parameters: { type: "object", properties: { to: { type: "string", description: "Recipient email address(es), separated by commas" }, subject: { type: "string", description: "Email subject line" }, body: { type: "string", description: "Email body content (plain text or HTML depending on content_type)" }, cc: { type: "string", description: "CC recipient(s), separated by commas" }, bcc: { type: "string", description: "BCC recipient(s), separated by commas" }, reply_to: { type: "string", description: "Reply-To email address" }, content_type: { type: "string", enum: ["text", "html"], description: "Content type: 'text' (default) or 'html'", default: "text" } }, required: ["to", "subject", "body"] },
      auth_type: "api_key",
      auth_config: { smtp_host: v.smtp_host, smtp_port: v.smtp_port || "587", smtp_user: v.smtp_user, smtp_password: v.smtp_password, from_address: v.from_address || v.smtp_user },
      timeout_sec: 30,
    }),
  },
