...
proto:
	protoc --proto_path=proto/worker/v1 \
		...
		worker.proto
	agent_engine/.venv/bin/python -m grpc_tools.protoc \
		-I proto/worker/v1 \
		--python_out=agent_engine/agent_engine \
		--grpc_python_out=agent_engine/agent_engine \
		proto/worker/v1/worker.proto
	sed -i 's/^import worker_pb2/from . import worker_pb2/' agent_engine/agent_engine/worker_pb2_grpc.py
